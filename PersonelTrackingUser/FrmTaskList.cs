...
namespace PersonelTrackingUser
{
    public partial class FrmTaskList : Form
    {
        ...
        private void FrmTaskList_Load(object sender, EventArgs e)
        {
            ...
            dataGridView1.Columns[5].HeaderText = "Deliver Date";
        ...
        private void btnSearch_Click(object sender, EventArgs e)
        {
            ...
            if (rbStartDate.Checked)
                list = list.Where(x => x.TaskStartDate > Convert.ToDateTime(dpStart.Value)
                        && x.TaskStartDate < Convert.ToDateTime(dpEnd.Value))
                        .ToList();
            if (rbDeliveryDate.Checked)
                list = list.Where(x => x.TaskDeliveryDate > Convert.ToDateTime(dpStart.Value)
                        && x.TaskDeliveryDate < Convert.ToDateTime(dpEnd.Value))
                        .ToList();
        ...
        {
            ExportToExcel.ExcelExport(dataGridView1, "Task");
        }
    }
}
